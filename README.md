# Juan Manuel Chuit Berzoni — Portfolio profesional

Portfolio personal orientado a mi perfil actual como **Analista Funcional Sr. / IT Business Analyst**, con foco en procesos, integraciones, productos digitales y mejora continua.

🌐 **Sitio:** https://jchuit.github.io/portfoliojchuit/

## Perfil

- Más de 5 años de experiencia en IT.
- Experiencia funcional en banca, fintech, oil & gas y ciberseguridad.
- Relevamiento y documentación de requerimientos.
- BPMN / UML y modelado AS IS / TO BE.
- Integraciones y APIs, validaciones end-to-end y Postman.
- SQL, Power BI, ServiceNow, Jira y Confluence.
- UAT, regresión, incidentes críticos y coordinación transversal.

## Stack principal

`BPMN` `UML` `SQL Server` `ASE / ISQL` `MySQL` `Oracle` `Postman` `APIs` `Jira` `Confluence` `ServiceNow` `Power BI` `AWS CloudWatch` `Splunk`

Metodologías y marcos: `Scrum` `Kanban` `ITIL` `BCRA` `PCI-DSS` `ISO 27001`.

## Archivo académico

La carpeta `Practicas/` conserva ejercicios de análisis de datos, Excel, SQL y visualización realizados durante etapas anteriores de formación. Se mantienen como archivo histórico y no representan el foco actual del portfolio.

## Contacto

- LinkedIn: https://www.linkedin.com/in/juan-manuel-chuit-berzoni/
- GitHub: https://github.com/jchuit
- Email: mjuan260@gmail.com

## Privacidad

El CV completo no se publica en el repositorio ni en el sitio para evitar exponer datos personales, como el teléfono. Para postulaciones se comparte por los canales de contacto indicados arriba.
